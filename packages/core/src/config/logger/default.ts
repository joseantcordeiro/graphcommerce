...
import { Logger, LogLevel, GraphCommerceLogger } from './graphcommerce';

const DEFAULT_CONTEXT = 'GraphCommerce';

/**
 * @description
 * The default logger, which logs to the console (stdout) with optional timestamps. Since this logger is part of the
 * default Graph Commerce configuration, you do not need to specify it explicitly in your server config. You would only need
 * to specify it if you wish to change the log level (which defaults to `LogLevel.Info`) or remove the timestamp.
 *
 * @example
 * ```ts
 * import { DefaultLogger, LogLevel, GraphCommerceConfig } from '\@graphcommerce/core';
 *
 * export config: GraphCommerceConfig = {
 *     // ...
 ...
 * @docsCategory Logger
 */
export class DefaultLogger implements GraphCommerceLogger {
    /** @internal */
