...
/**
 * @description
 * The ServerLogger interface defines the shape of a logger service which may be provided in
 * the config.
 *
 * @docsCategory Logger
 */
export interface GraphCommerceLogger {
...
const noopLogger: GraphCommerceLogger = {
    error() {
        /* */
    },
    warn() {
...
/**
 * @description
 * The Logger is responsible for all logging in a Graph Commerce Server application.
 *
 * It is intended to be used as a static class:
 *
 * @example
 * ```ts
 * import { Logger } from '\@graphcommerce/core';
 *
 * Logger.info(`Some log message`, 'My Graph Commerce Plugin');
 * ```
 *
 * The actual implementation - where the logs are written to - is defined by the {@link GraphCommerceLogger}
 * instance configured in the {@link GraphCommerceConfig}. By default, the {@link DefaultLogger} is used, which
 * logs to the console.
 *
 * ## Implementing a custom logger
 *
 * A custom logger can be passed to the `logger` config option by creating a class which implements the
 * {@link GraphCommerceLogger} interface. For example, here is how you might go about implementing a logger which
 * logs to a file:
 *
 * @example
 * ```ts
 * import { GraphCommerceLogger } from '\@graphcommerce/core';
 * import fs from 'fs';
 *
 * // A simple custom logger which writes all logs to a file.
 * export class SimpleFileLogger implements GraphCommerceLogger {
...
export class Logger implements LoggerService {
    ...
    private static _logger: GraphCommerceLogger;

    static get logger(): GraphCommerceLogger {
    ...
    /** @internal */
    static useLogger(logger: GraphCommerceLogger) {
